...
docker pull $IMAGE
docker run --rm -v $(pwd):$(pwd) -w $(pwd) --env CHANGED_PATHS="${CHANGED_PATHS}" --env-file env.list $IMAGE go run ci/pipeline/cmd/pipeline.go
...
buildkite-agent pipeline upload cpipeline.yaml
